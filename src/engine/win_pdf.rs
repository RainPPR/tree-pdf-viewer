...
impl PdfEngine for WinPdfEngine {
    fn open(&mut self, path: &Path) -> Result<()> {
        ...
        let file = StorageFile::GetFileFromPathAsync(&windows::core::HSTRING::from(path_str.as_ref()))?.get()?;
    ...
    fn page_count(&self) -> usize {
        self.document.as_ref().map(|d| d.PageCount().unwrap_or(0) as usize).unwrap_or(0)
    }
