...
impl PdfEngine for PdfiumEngine {
    fn open(&mut self, path: &Path) -> Result<()> {
        ...
        self.document = Some(unsafe { std::mem::transmute::<PdfDocument<'_>, PdfDocument<'static>>(doc) });
        Ok(())
    }
    ...
    fn page_count(&self) -> usize {
        self.document.as_ref().map(|d| d.pages().len() as usize).unwrap_or(0)
    }
...
impl PdfiumEngine {
    fn parse_bookmarks<'a>(&self, mut iter: PdfBookmarksIterator<'a>, level: usize) -> Vec<TocItem> {
        let mut items = vec![];
        
        while let Some(bm) = iter.next() {
            let children = self.parse_bookmarks(bm.iter_direct_children(), level + 1);
            
            let page_index = bm.destination()
